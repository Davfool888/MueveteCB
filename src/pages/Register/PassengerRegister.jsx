...
export default function PassengerRegister() {
  ...
  function validateStep2() {
    const errs = {};
    if (!form.localidad) errs.localidad = 'Selecciona una localidad.';
    if (form.transportes.length === 0)
      errs.transportes = 'Selecciona al menos un medio de transporte.';
    return errs;
  }
  ...
  if (submitted) {
    return (
      <div className="page-shell">
        <Header showRegisterLink={false} customActions={<BackToSelectButton />} />
        <main className="register-main">
          <div className="reg-success-card">
            <div className="reg-success-icon" aria-hidden="true">✅</div>
            <h1>¡Registro completado!</h1>
            <p>
              Bienvenido/a, <strong>{form.nombre} {form.apellido}</strong>.<br />
              Tu cuenta de <strong>Pasajero</strong> ha sido creada correctamente.
            </p>
  ...
  return (
    <div className="page-shell">
      <a className="skip-link" href="#reg-passenger-content">Saltar al contenido principal</a>
      <Header showRegisterLink={false} customActions={<BackToSelectButton />} />

      <main id="reg-passenger-content" className="register-main" style={{ alignItems: 'flex-start' }}>
        <div className="reg-form-shell">
          {/* cabecera */}
          <div className="reg-form-head">
            <span className="section-kicker">Registro · Pasajero</span>
            <h1>
          ...
          {step === 1 && (
            <form className="reg-form" noValidate onSubmit={(e) => { e.preventDefault(); handleNext(); }}>
              <div className="reg-field-row">
                <FormField id="nombre" label="Nombre" value={form.nombre} onChange={(v) => set('nombre', v)} error={errors.nombre} placeholder="Ej. María" required />
                <FormField id="apellido" label="Apellido" value={form.apellido} onChange={(v) => set('apellido', v)} error={errors.apellido} placeholder="Ej. Vargas" required />
              </div>
              ...
              <FormField id="telefono" label="Número de teléfono" type="tel" value={form.telefono} onChange={(v) => set('telefono', v)} error={errors.telefono} placeholder="Ej. 3001234567" hint="Solo números, sin espacios ni guiones." required />
          ...
          {step === 2 && (
            <form className="reg-form" noValidate onSubmit={(e) => { e.preventDefault(); handleNext(); }}>
              ...
              <FormField id="barrio" label="Barrio o sector" value={form.barrio} onChange={(v) => set('barrio', v)} placeholder="Ej. Lucero Bajo, Candelaria la Nueva" hint="Opcional — te ayuda a personalizar tus rutas." />

              <div className="reg-field-col">
                <span className="reg-label">
                  Medios de transporte que usas <span className="reg-required">*</span>
                </span>
                <div className="transport-selector-grid">
                  {PASSENGER_TRANSPORT_OPTIONS.map((opt) => {
                    const active = form.transportes.includes(opt.id);
                    return (
                      <button
                        key={opt.id}
                        type="button"
                        className={`transport-option-pill${active ? ' is-selected' : ''}`}
                        onClick={() => toggleTransporte(opt.id)}
                        aria-pressed={active}
                ...
                {errors.transportes && <span className="reg-error">{errors.transportes}</span>}
              </div>
          ...
          {step === 3 && (
            <form className="reg-form" onSubmit={handleSubmit}>
              <div className="reg-summary">
                ...
                <SummaryRow label="Barrio / sector" value={form.barrio || '—'} />
                <SummaryRow
                  label="Transportes"
                  value={
                    form.transportes.length
                      ? PASSENGER_TRANSPORT_OPTIONS
                          .filter((o) => form.transportes.includes(o.id))
                          .map((o) => o.label)
                          .join(', ')
                      : '—'
                  }
                ...
                <div className="reg-summary-note">
                  <span aria-hidden="true">🔒</span> Tu contraseña está cifrada y nunca se mostrará en este resumen.
                </div>
              </div>
